...
function initSlider() {
    ...
    function initList() {
      images.forEach((image, index) => {
        let item = `<div class="slider__list-item n${index} ${index === 0? "active" : ""}" data-index="${index}">${images[index].city}</div>`;
        sliderList.innerHTML += item;
      });
      sliderList.querySelectorAll(".slider__list-item").forEach(item => {
